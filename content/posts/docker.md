...
description = "Containers and security"
date = 2019-04-25
...
called *containers*. It leverages OS-level virtualisation technologies
...
while VBoxes for example use each a completely separate stack.

This allows Docker containers to : 

 - Run extremely fast, because they don't have to boot up or shut down
 - Take less HDD space ([vanilla debian weighs less than 50MB][4])
...
You can also search this on the [Hub Webpage](https://hub.docker.com)
or if you use [DuckduckGo](https://ddg.gg), using the associated `!docker` bang.
The web results will give you more detailed information (image building process,
GitHub repo, and so on) than just a plain CLI search without arguments.
...
_Dockerfiles_. In these files, you specify a base image to start `FROM`, then
...
[dref]: https://docs.docker.com/engine/reference/dockerfile
[bprc]: https://docs.docker.com/build/building/best-practices/
...
[dc-options]: https://docs.docker.com/reference/cli/docker/container/run/
[dc-example]: https://gist.github.com/blackstorm/d446814539daace544d0c9c61e842d18
[dc-source]: https://github.com/docker/compose/releases/
...
Let's call it "okay practice" instead and define it in simple terms :
 ...
 - Don't blindly trust your base image. Check the build and the runtime
	...
	You can use inspection tools such as [Lynis][S1], [Grype][S2] and
	[OpenVAS][S3], and enforcement tools such as [AppArmor][S4],
	[SELinux][S5], and [AIDE][S7].
...
[S2]: https://github.com/anchore/grype
[S3]: http://openvas.org/
[S4]: https://wiki.ubuntu.com/AppArmor
[S5]: https://selinuxproject.org/
...
trust zones and securing your accesses, as Docker runs with near-root
